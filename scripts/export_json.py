...
async def main() -> None:
    ...
    for beach in beaches:
        ...
        try:
            ...
            agitation = classify_agitation(wave_height_m)
    ...
    # Praia recomendada do dia: entre as com balneabilidade própria,
    # prioriza as marcadas como "familiar" no cadastro do INEA (se houver
    # alguma); dentro desse grupo (ou de todas as próprias, se nenhuma for
    # marcada como familiar), a de maior score.
    proprias = [(bid, e) for bid, e in praias_json.items() if e["balneabilidade"] == "propria"]
    candidatas = proprias if proprias else list(praias_json.items())

    familiares = [(bid, e) for bid, e in candidatas if "familiar" in e["caracteristicas"]]
    if familiares:
